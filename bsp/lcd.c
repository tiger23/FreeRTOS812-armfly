...
* History:
* 02/01/2015
* This file port to the LCD driver for SPFD5420A
* liquid Crystal Display Module of STM32F10x-ARMFLY.
* by astiger
* tiger.w@outlook.com
********************************************************************************/
...
void LCD_Init(void)
{
    ...
    LCD_WriteReg(R000, 0x0000);
    LCD_WriteReg(R001, 0x0100);
    LCD_WriteReg(R002, 0x0100);
    //vTaskDelay( 10 / portTICK_PERIOD_MS ); /* Delay 10 ms */
    LCD_WriteReg(R003, 0x1018);
    LCD_WriteReg(R008, 0x0808);

    LCD_WriteReg(R009, 0x0001);
    LCD_WriteReg(R00B, 0x0010);
    LCD_WriteReg(R00C, 0x0000);
    LCD_WriteReg(R00F, 0x0000);
    LCD_WriteReg(R007, 0x0001);

    /* LCD Power On --------------------------------------------------------------*/
    LCD_WriteReg(R010, 0x0013);
    LCD_WriteReg(R011, 0x0501);
    LCD_WriteReg(R012, 0x0300);
    LCD_WriteReg(R020, 0x021E);
    //vTaskDelay( 10 / portTICK_PERIOD_MS ); /* Delay 10 ms */
    LCD_WriteReg(R021, 0x0202);
    LCD_WriteReg(R090, 0x8000);
    //vTaskDelay( 60 / portTICK_PERIOD_MS ); /* Delay 60 ms */
    LCD_WriteReg(R100, 0x17B0);
    //vTaskDelay( 60 / portTICK_PERIOD_MS ); /* Delay 60 ms */
    LCD_WriteReg(R101, 0x0147);
    //vTaskDelay( 10 / portTICK_PERIOD_MS ); /* Delay 10 ms */
    LCD_WriteReg(R102, 0x0135);
    LCD_WriteReg(R107, 0x0000);
    LCD_WriteReg(R110, 0x0001);
    //vTaskDelay( 10 / portTICK_PERIOD_MS ); /* Delay 10 ms */

    /* Chip Set ------------------------------------------------------------------*/
    LCD_WriteReg(R210, 0x0000);
    LCD_WriteReg(R211, 0x00ef);

    LCD_WriteReg(R212, 0x0000);
    LCD_WriteReg(R213, 0x018F);
    LCD_WriteReg(R280, 0x0000);
    LCD_WriteReg(R281, 0x0004);
    LCD_WriteReg(R282, 0x0000);
    LCD_WriteReg(R300, 0x0101);
    LCD_WriteReg(R301, 0x0B2C);
    LCD_WriteReg(R302, 0x1030);
    LCD_WriteReg(R303, 0x3010);
    LCD_WriteReg(R304, 0x2C0B);

    LCD_WriteReg(R305, 0x0101);
    LCD_WriteReg(R306, 0x0807);

    LCD_WriteReg(R307, 0x0708);
    LCD_WriteReg(R308, 0x0107);
    LCD_WriteReg(R309, 0x0105);
    LCD_WriteReg(R30A, 0x0F04);
    LCD_WriteReg(R30B, 0x0F00);

    LCD_WriteReg(R30C, 0x000F);
    LCD_WriteReg(R30D, 0x040F);
    LCD_WriteReg(R30E, 0x0300);
    LCD_WriteReg(R30F, 0x0701);
    LCD_WriteReg(R400, 0x3500);
    LCD_WriteReg(R401, 0x0001);
    LCD_WriteReg(R404, 0x0000);
    LCD_WriteReg(R500, 0x0000);
    LCD_WriteReg(R501, 0x0000);
    LCD_WriteReg(R502, 0x0000);
    LCD_WriteReg(R503, 0x0000);
    LCD_WriteReg(R504, 0x0000);
    LCD_WriteReg(R505, 0x0000);
    LCD_WriteReg(R600, 0x0000);
    LCD_WriteReg(R606, 0x0000);

    LCD_WriteReg(R6F0, 0x0000);  /* R29:Gate scan direction setting */

    LCD_WriteReg(R7F0, 0x5420);
    LCD_WriteReg(R7DE, 0x0000);
    LCD_WriteReg(R7F2, 0x00DF);
    LCD_WriteReg(R7F3, 0x0810);

    LCD_WriteReg(R7F4, 0x0077);  /* R141:Register set-up mode for one line clock */
    LCD_WriteReg(R7F5, 0x0021);  /* R139:One line SYSCLK number in one-line */
    LCD_WriteReg(R7F0, 0x0000);  /* R51:N line inversion setting */
    LCD_WriteReg(R007, 0x0173);  /* R55:Scanning method setting */
    ...
    /* Display On ----------------------------------------------------------------*/
    LCD_WriteReg(R210, 0);
    LCD_WriteReg(R211, 239);

    LCD_WriteReg(R212, 0);
    LCD_WriteReg(R213, 399);
...
{
    uint32_t i = 0, column = 0, index = 0, spaceindex = 0;
    uint16_t refcolumn = 399;
    uint32_t length = 0;
...
void LCD_SetDisplayWindow(uint8_t Xpos, uint16_t Ypos, uint8_t Height, uint16_t Width)
{
    ...
    */
    ...
    LCD_WriteReg(R210, px);
    LCD_WriteReg(R211, (px + Height - 1));

    LCD_WriteReg(R212, (py - Width + 1));
    LCD_WriteReg(R213, py);
...
void LCD_PowerOn(void)
{
    /* Power On Set */
    LCD_WriteReg(R007, 0x0001);
    LCD_WriteReg(R110, 0x0001); /* PSE */
    LCD_WriteReg(R100, 0x0730); /* BT,AP 0x0330　*/
    LCD_WriteReg(R101, 0x0237); /* DC0,DC1,VC */
    LCD_WriteReg(R103, 0x2b00); /* VDV  //0x0f00 */
    LCD_WriteReg(R280, 0x4000); /* VCM */
    LCD_WriteReg(R102, 0x81b0); /* VRH,VCMR,PSON,PON */
